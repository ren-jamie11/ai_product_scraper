...
- communicates what success looks like, and leaves how to achieve it to the model;
...
- whether the requested level of specificity is appropriate, and whether any instructions dictate implementation details the model could decide for itself;
...
### 3. Specify outcomes, not implementation

Focus the prompt on the desired functionality, results, and objectives. Trust the model to handle technical implementation once the goal is clearly communicated.

- Describe what the result should do, look like, or achieve, and why it matters.
- Do not prescribe algorithms, data structures, thresholds, tie-breakers, parsing rules, file layouts, or step-by-step technical procedures unless the draft already requires them or they are true business constraints.
- Express quality expectations as outcomes (e.g. "a wrong match is worse than no match") rather than as mechanisms (e.g. "set a confidence threshold and suppress results below it").
- When you notice a technical risk the model might miss, state it briefly as something to check or flag, not as a prescribed solution.

If a polished prompt reads like a technical spec, it is over-specified. Cut it back to the objectives.

### 4. Turn implicit expectations into explicit decision rules

Identify places where the desired behavior is understandable to a human but underspecified for an AI. Keep these rules at the level of intended behavior and priorities, not implementation.
...
### 5. Distinguish semantic intent from surface wording
...
### 6. Make examples function correctly
...
### 7. Tighten ambiguity without over-specifying

Add enough instruction to resolve meaningful uncertainty, but do not create dozens of unnecessary rules.

Prefer a small number of high-value principles over exhaustive micromanagement. Brevity is a feature: every added rule should earn its place by changing the result.

### 8. Improve terminology
...
### 9. Preserve flexibility where judgment is useful

Do not make the prompt so rigid that it prevents reasonable model judgment. Implementation choices in particular should usually be left to the model.
...
Do not add these mechanically. Include only what improves the specific prompt, and phrase additions as objectives or expectations rather than technical instructions.
...
- professional;
- practical;
- specific about goals, light on implementation;
...
**Preserve first, clarify second, strengthen where useful, and only expand when the expansion improves model behavior. Define the what and the why; trust the model with the how.**
